...
description: 在這個追求極致網頁效能的時代，我們不斷追求如何讓網頁更輕量且高效。在這篇文章中，我將分享如何在 Nuxt 中使用 html-minifier-terser 處理 HTML 壓縮，盡可能的提升網頁加載速度。如果你也正在使用 Nuxt 架設網站，相信這篇內容會對你有幫助，那就讓我們一起看下去吧！
---

## 前言

這篇會以我如和壓縮這個部落格的靜態 HTML 為例。如果是熟悉 Vue 的朋友想要自架部落格，我會首推選用 Nuxt Content，Nuxt Content 簡單易用好上手，輕輕鬆鬆就可以打造出具有個人風格的網站。
...
事實上 Nuxt 已經是一個非常高效的框架，但觀察了 Nuxt 生成的 HTML 檔案後還是發現了一些可以改進的地方。下圖 HTML 是未壓縮前的樣子。
...
根據 Nuxt 2 [文件](https://v2.nuxt.com/docs/configuration-glossary/configuration-build/#htmlminify)中提到的設定，可以透過在 `nuxt.config.js` 中加入這段設定來壓縮 HTML：
...
但在 Nuxt 3 的文件跟原始碼裡面並沒有找到相關的設定，看起來我們必須自己動手處理。

經過幾次嘗試後覺得最簡單的方式就是使用 `html-minifier-terser` 這個工具幫助我們處理 HTML 壓縮，而且也有許多設定可以調整。接下來就來看看如何將 `html-minifier-terser` 整合進 Nuxt 進行 HTML 壓縮。

## Nuxt 實作 HTML 壓縮

首先我們得先安裝 `html-minifier-terser`：
...
**另外一個方式則是透過 Nuxt 提供的 Lifecycle Hooks 來處理。**
...
但要解決前兩個問題可能就要腦力激盪一下了，為什麼 `<!DOCTYPE html>` 跟 `<html>` 以及 `<head>` 之間有個換行？為什麼 `<html  lang="zh-TW">` 跟 `<body >` 都多了一個空白？
...
舉例來說這是一筆 `html` 的資料（Nuxt 3 Playground）
...
想要維持現狀並且同時解決這個問題可能就要要到 Nuxt 底層的原始碼去修改了。

除此之外我們還有沒有其他方法可以解決掉這兩個問題呢？使用 `render:response` 這個 hooks 試試看吧！不囉唆先看結果：
...
要理理解這裡在做什麼首先要知道 `response` 裡面有哪些資料，下列是 `response` 的型別定義：
...
其中裡面的 `body` 就是執行 `renderHTMLDocument(htmlContext)` 的結果。
